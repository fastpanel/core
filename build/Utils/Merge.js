...
class Merge {
     ...
     * @return any
     */
    static merge(clone, recursive, ...argv) {
        let result = argv[0], size = argv.length;
        if (clone || Merge.typeOf(result) !== 'object')
            result = {};
        for (let index = 0; index < size; ++index) {
            let item = argv[index], type = Merge.typeOf(item);
            if (type !== 'object')
                continue;
            for (let key in item) {
     ...
     */
    static clone(input) {
        let output = input, type = Merge.typeOf(input), index, size;
        if (type === 'array') {
            output = [];
            size = input.length;
